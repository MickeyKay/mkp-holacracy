...
class MKP_Holacracy_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( "{$this->plugin_slug}-admin", plugin_dir_url( __FILE__ ) . "css/mkp-holacracy-admin{$min_suffix}.css", array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		$min_suffix = $this->plugin->get_min_suffix();
		wp_enqueue_script( "{$this->plugin_slug}-admin", plugin_dir_url( __FILE__ ) . "js/mkp-holacracy-admin{$min_suffix}.js", array( 'jquery' ), $this->version, false );
	...
	public function add_settings_page() {
		...
		$this->settings_page = add_options_page(
			__( 'MKP Holacracy', 'mkp-holacracy' ), // Page title
			__( 'MKP Holacracy', 'mkp-holacracy' ), // Menu title
	...
	public function do_settings_page() {
		...
		<div class="wrap <?php echo $this->plugin_slug; ?>-settings">
	        ...
	        <h2 class="nav-tab-wrapper">
	        	<a href="<?php echo $tab_base_url; ?>&tab=tab-1" class="nav-tab <?php echo ( ! $active_tab || 'tab-1' == $active_tab ) ? 'nav-tab-active' : ''; ?>"><?php esc_html_e( 'Tab 1', 'mkp-holacracy' ); ?></a>
	        	<a href="<?php echo $tab_base_url; ?>&tab=tab-2" class="nav-tab <?php echo ( 'tab-2' == $active_tab ) ? 'nav-tab-active' : ''; ?>"><?php esc_html_e( 'Tab 2', 'mkp-holacracy' ); ?></a>
	        </h2>
	...
	public function add_settings_fields() {
		...
		add_settings_field(
			$id, // ID
			__( 'Option 1', 'mkp-holacracy' ), // Title
			array( $this, 'render_checkbox' ), // Callback
			"{$this->plugin_slug}-{$tab}", // Page
			$tab, // Section
			array( // Args
				'id'          => $id,
				'description' => __( 'This is option 1.', 'mkp-holacracy' ),
				'save_null'   => false,
			)
		);
		...
		add_settings_field(
			$id, // ID
			__( 'Option 2', 'mkp-holacracy' ), // Title
			array( $this, 'render_checkbox' ), // Callback
			"{$this->plugin_slug}-{$tab}", // Page
			$tab, // Section
			array( // Args
				'id'          => $id,
				'description' => __( 'This is option 2.', 'mkp-holacracy' ),
				'save_null'   => false,
			)
		);
	...
	function edit_admin_columns( $columns ) {
		$columns['status'] = __( 'Status', 'mkp-holacracy' );
		return $columns;
	}

	public function do_custom_column_output( $column, $post_id ) {

		switch ( $column ) {

			case 'status':
				$status = get_post_meta( $post_id, "_{$this->plugin_slug}_status", true );
				printf( '<span class="mkp-status-indicator %s"></span>', $status );
				break;
		}
